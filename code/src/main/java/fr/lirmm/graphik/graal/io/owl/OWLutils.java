...
public class OWLutils {
	...
	public static OWLClassExpression classExpressionDisjunctiveNormalForm(
		...
		if (classExpression instanceof OWLObjectUnionOf) {
			...
			for (OWLClassExpression element : OWLutils
					.getObjectUnionOperands(classExpression)) {
				element = classExpressionDisjunctiveNormalForm(element);
				for (OWLClassExpression e : OWLutils
						.getObjectUnionOperands(element)) {
					union.add(e);
				}
			}
			List<OWLClassExpression> list = List.copyOf(union);
			return new OWLObjectUnionOfImpl(list);
			...
			if (conjunctions.size() > 1) {
				for (Set<OWLClassExpression> conj : conjunctions) {
					List<OWLClassExpression> expressionConj = new ArrayList<>(conj);
					union.add(new OWLObjectIntersectionOfImpl(expressionConj));
				}
				List<OWLClassExpression> expressionUnion = new ArrayList<>(union);
				return new OWLObjectUnionOfImpl(expressionUnion);
			} else {
				List<OWLClassExpression> newConj = List.copyOf(conjunctions.get(0));
				return new OWLObjectIntersectionOfImpl(newConj);
			}
			...
			if (filler instanceof OWLObjectUnionOf) {
				...
				List<OWLClassExpression> expressionList = new ArrayList<>(union);
				return new OWLObjectUnionOfImpl(expressionList);
			}
			...
			if (filler instanceof OWLDataUnionOf) {
				Set<OWLClassExpression> union = new TreeSet<>();
				for (OWLDataRange e : OWLutils
						.getDataUnionOperands(filler)) {
					e = dataRangeDisjunctiveNormalForm(e);
					union.add(new OWLDataSomeValuesFromImpl(prop, e));
				}
				List<OWLClassExpression> expressionUion = new ArrayList<>(union);
				return new OWLObjectUnionOfImpl(expressionUion);
			}
			...
			for (OWLIndividual i : expr.getIndividuals()) {
				...
				Stream<OWLIndividual> individualStream = Stream.of(i);
				union.add(new OWLObjectOneOfImpl(individualStream));
			}
			List<OWLClassExpression> expressionUion = new ArrayList<>(union);
			return new OWLObjectUnionOfImpl(expressionUion);
	...
	public static OWLDataRange dataRangeDisjunctiveNormalForm(
		...
		if (dataRange instanceof OWLDataUnionOf) {
			Set<OWLDataRange> union = new TreeSet<>();
			for (OWLDataRange element : OWLutils
			...
			List<OWLDataRange> expressionUion = new ArrayList<>(union);
			return new OWLDataUnionOfImpl(expressionUion);
			...
			if (conjunctions.size() > 1) {
				for (Set<OWLDataRange> conj : conjunctions) {
					List<OWLDataRange> listConj = new ArrayList<OWLDataRange>(conj);
					union.add(new OWLDataIntersectionOfImpl(listConj));
				}
				return new OWLDataUnionOfImpl((List<OWLDataRange>) union);
			} else {
				return new OWLDataIntersectionOfImpl((List<OWLDataRange>) conjunctions.get(0));
			}
